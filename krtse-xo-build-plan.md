...
The About page background, and a quieter version behind the home hero. The brief: sitting by a lake before sunrise. Calm, small movement, and now and then a bird crosses or a fish jumps.

**Deliberate override of the original tuning rule.** The spec's instinct — "if it reads as an animation, the amplitude is too high" — was followed and produced water nobody could see move at all, across three review passes. The first fix guessed the cause was amplitude and raised it; that changed nothing perceptible, because it was never amplitude. Each band is a *filled* shape stacked on the next, so what should read as "a wave" is the boundary between two nearly identical translucent fills — at spec alpha values, neighbouring bands differ by well under 1% opacity, roughly a 1-in-255 colour step. No distance travelled makes an invisible edge visible; raising amplitude just moves the invisible edge further.

The actual fix is contrast, not distance: each band's surface gets a **stroked crest line**, at several times the fill's alpha, on top of the fill. That line is what your eye actually tracks. Once it existed, amplitude and fill alpha could go back to close to the original spec values — they were never the problem.

- **Canvas 2D**, no WebGL. Four stacked bands, each the sum of two or three sines with different frequencies, phases and drift speeds, **plus a stroked crest line along the surface at ~6× the fill's alpha** (~1.25px). Amplitude 10–24px (close to the original 6–14px). Full period 10–33s — roughly half the original 20–45s, so the crest line's motion is perceptible rather than glacial; this is the other half of the real fix, since a correctly-visible but near-static line still doesn't read as "moving water."
- **Color:** `--signal` at 5–7% fill alpha (`--water-alpha` in globals.css, back at its original 0.07/0.05 — a previous pass raised it chasing visibility, which never worked and cost About page's measured text-contrast margin for nothing; reverted once the crest line made that unnecessary), band nearest the horizon slightly lighter. The crest line itself runs several times that alpha, per band.
- **Rare events:** an ambient ripple ring expands and fades at a random point every 16–36s, dissipating over 4.5s, radius up to 170px (originally 25–60s / 110px, widened for legibility). A bird silhouette crosses the upper area every 60–120s, taking ~18s, at 0.32 opacity (originally 0.12 — same invisibility bug as the bands, same fix). **A fish jumps** — a real parabolic leap with a silhouette, not a stand-in ripple — every 7–15s (the first one on the home hero uses a shorter 3–6s delay, long enough to clear the dandelion intro without a long wait), with its own splash ripple at both the point it leaves the water and where it lands. Random intervals inside a range, never a fixed loop.
- **Text sits still on top.** Canvas is `pointer-events: none`, behind everything.
- **Performance:** rAF capped at 30fps, paused by `IntersectionObserver` when offscreen and on `document.hidden`, DPR capped at 2, debounced resize. Reduced motion draws one static frame (bands and crest lines, no ripple/bird/fish — those are only ever scheduled from inside the loop reduced motion skips) and stops.

A reference implementation exists in `krtse-xo-prototype.html` — read it before building the component. (That file was never actually in this repo; the component here was built and iteratively fixed against this spec and direct visual review instead.)
